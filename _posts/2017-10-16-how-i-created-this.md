...
Three months after buying the shirish.me domain name, I decided to make a good-looking blog. Since I'm a comically bad designer, I needed an inspiration. Fortunately for me, [Julia Evans's website](https://jvns.ca) was a site I regularly visited, so that's where I began.
...
Now that I had my development environment set, I needed to get the blog. Since I had decided to only use the layout and not the internals of Julia's website, I ran a `curl -p -k https://jvns.ca` to copy her homepage in its entirety to my workspace. Since her blog runs on Jekyll too, all I had to to was `$ jekyll serve --host $IP --port $PORT --baseurl ''` and I was ready, serving my (stolen) Jekyll blog.
